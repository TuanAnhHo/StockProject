...
class CrawlStockPriceBySymbol:
    ...
    def ConvertDateToSeconds(self, input_date:str):
        return int(datetime.strptime(input_date,'%Y-%m-%d').timestamp()) + (7*60*60) ## Convert 7 hours to seconds to plus to input date because API response time default +7 hours
    ...
    def CrawlDataResponse(self):
        ...
        try:
            ...
            response = requests.get(
                url=url_base+"resolution=D&symbol={0}&from={1}&to={2}".format(self.symbol,self.RangeTimeConfig["from_date_convert"],self.RangeTimeConfig["to_date_convert"]),                                                                        
                headers=headers
            )
